...
execute unless entity @p[predicate=legitieconomy:match_uuid] run return run kill @s
execute on target if entity @s[predicate=legitieconomy:match_uuid] run return run function legitieconomy:cancel_beg
scoreboard players operation .temp beg = @s beg

execute if score @p[predicate=legitieconomy:match_uuid] le.beggingTimer > .beggingInteractionThreshold le.beggingTimer run return fail
...
tag @p[predicate=legitieconomy:match_uuid] add .temp

execute on target run function legitieconomy:send_money
execute as @p[tag=.temp] run function legitieconomy:receive_money
execute as @p[predicate=legitieconomy:match_uuid] run function legitieconomy:cancel_beg
